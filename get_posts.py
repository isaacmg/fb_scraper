...
def scrape_posts_from_last_scrape(id):
    scrape_groups_pages(id, 1, 1,  False)

# You must have a Kafka running on localhost:9092 or change the port in fb_posts_realtime.py
...
def scrape_posts_from_last_scrape_kafka(id):
    scrape_groups_pages(id, 1, 0, False)

def scrape_all_posts(id):
    scrape_groups_pages(id, 0, 1, False)

def scrape_all_comments(id):
    scrape_groups_pages(id, 0, 2, True)
